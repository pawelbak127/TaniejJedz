"""
Manual test — real Wolt API.
Usage:
  cd backend
  $env:REDIS_URL="redis://:localdevpassword@localhost:6379/0"
  python -m app.scraper.tests.manual_test_wolt
"""
...
async def main() -> None:
    ...
    for i, r in enumerate(restaurants[:10], 1):
        ...
        print(f"      slug: {r.platform_slug} | ETA: {eta} | rating: {rating}")

    # Find first open restaurant
    ...
    if not menu_items:
        print("(puste menu — spróbuj inną restaurację)")
        # Try next restaurant
        for r in restaurants[1:5]:
            if r.is_online:
                print(f"\nPróbuję: {r.platform_name} ({r.platform_slug})")
                try:
                    menu_items = await adapter.get_menu(r.platform_slug)
    ...
    if not menu_items:
        print("Nie udało się pobrać menu z żadnej restauracji.")
        await redis.aclose()
        return
